...
fn get_current_width_and_group(s: &mut Cursive) -> (u64, u16) {
    s.call_on_name("hex_view", |v: &mut HexView| {
...
fn do_set_widths(s: &mut Cursive) {
    let line_width = s.call_on_name("line_width", get_content).unwrap();
    let group = s.call_on_name("group", get_content).unwrap();

    s.pop_layer();

    if !line_width.is_empty() {
        match parse_number(&line_width) {
            Ok(width) => s.call_on_name("hex_view", |v: &mut HexView| {
                if width > 0 {
    ...
    if !group.is_empty() {
        match parse_number(&group) {
            Ok(group) => s.call_on_name("hex_view", |v: &mut HexView| {
