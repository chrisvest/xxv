    ...
    let layout = LinearLayout::vertical()
        .child(
            TextView::new("")
                .center()
                .effect(Effect::Bold)
                .with_name("current_dir"),
        )
        .child(DummyView)
        .child(
            LinearLayout::horizontal()
                .child(ScrollView::new(
                    dir_selector.with_name("dir_selector").full_width(),
                ))
                .child(ScrollView::new(
                    file_selector.with_name("file_selector").full_width(),
                )),
        )
...
fn select_directory(s: &mut Cursive, dir: &OsStr) {
    let mut current_dir = s.find_name::<TextView>("current_dir").unwrap();
    let mut dir_selector = s.find_name::<SelectView<OsString>>("dir_selector").unwrap();
